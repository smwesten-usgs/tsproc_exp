...
export RANLIBLIST=$( locate ranlib-5 | grep Cellar | grep bin | grep $GCC_VERSION )
export RANLIBARR=($RANLIBLIST)
export RANLIB=${RANLIBARR[1]}

export ARLIST=$( locate ar-5 | grep Cellar | grep bin | grep $GCC_VERSION )
export ARARR=($ARLIST)
export AR=${ARARR[1]}
